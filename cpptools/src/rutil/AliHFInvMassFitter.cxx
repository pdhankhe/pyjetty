...
#include "AliHFInvMassFitter.hh"


ClassImp(RUtil::AliHFInvMassFitter)
...
void AliHFInvMassFitter::InvMassFitter(const TH1F * histoToFit, Double_t minvalue, Double_t maxvalue, Int_t fittypeb, Int_t fittypes){
    /// standard constructor
    fHistoInvMass=(TH1F*)histoToFit->Clone("fHistoInvMass");
    fHistoInvMass->SetDirectory(0);
    SetNumberOfParams();
...
void AliHFInvMassFitter::SetNumberOfParams(){
  ...
  case 6:
    ...
    break;
  default:
          std::cout<<"Error in computing fNParsBkg: check fTypeOfFit4Bkg"<<std::endl;
    break;
  }
  ...
  case 2:
    ...
    break;
  default:
          std::cout<<"Error in computing fNParsSig: check fTypeOfFit4Sgn"<<std::endl;
    break;
  }
...
TF1* AliHFInvMassFitter::CreateBackgroundFitFunction(TString fname, Double_t integral){
  ...
  case 6:
    ...
    break;
  default:
          std::cout<<Form("Wrong choice of fTypeOfFit4Bkg (%d)",fTypeOfFit4Bkg)<<std::endl;
    delete funcbkg;
    return 0x0;
    break;
  }
...
TF1* AliHFInvMassFitter::CreateReflectionFunction(TString fname){
  ...
  /// in the D0->Kpi inv. mass distribution
   // std::cout<<"checking reflection function creation"<<&AliHFInvMassFitter::FitFunction4Refl<<std::endl;
...
Double_t AliHFInvMassFitter::FitFunction4Refl(Double_t *x,Double_t *par){
  ...
  if(!fHistoTemplRfl) return 0;
    //std::cout<<"reflection find bin"<<x[0]<<std::endl;
  //std::cout<<"reflection find bin"<<fHistoTemplRfl->GetBinContent(2)<<std::endl;
...
//_________________________________________________________________________
double* AliHFInvMassFitter::Signal(Double_t min, Double_t max) const {
  /// Return signal integral in a range
  ///
  double* signal_info = new double [2];
  signal_info[0]=fSigFunc->Integral(min, max)/(Double_t)fHistoInvMass->GetBinWidth(1);
  std::cout<<"Signal from the invmass class is" << signal_info[0] <<std::endl;
  signal_info[1]=(fRawYieldErr/fRawYield)*(signal_info[0]);/*assume relative error is the same as for total integral*/
...
void AliHFInvMassFitter::Background(Double_t nOfSigma,Double_t &background,Double_t &errbackground) const {
  ...
  double* output_bkg = new double [2];
  output_bkg = Background(minMass,maxMass);
  background = output_bkg[0];
  errbackground = output_bkg[1];
  return;

}

//___________________________________________________________________________
double* AliHFInvMassFitter::Background(Double_t min, Double_t max) const {
  /// Return background integral in a range
  ///

  double* background_info = new double [2];
  TF1 *funcbkg=0x0;
  if(fBkgFuncRefit) funcbkg=fBkgFuncRefit;
  else if(fBkgFunc) funcbkg=fBkgFunc;
  if(!funcbkg){
      std::cout<<"Bkg function not found!"<<std::endl;
  ...
  background_info[0]=funcbkg->Integral(min,max)/(Double_t)fHistoInvMass->GetBinWidth(1);
  background_info[1]=intBerr/intB*background_info[0];
...
void AliHFInvMassFitter::Significance(Double_t nOfSigma,Double_t &significance,Double_t &errsignificance) const  {
  ...
  double* output_significance = new double [2];
  output_significance = Significance(minMass, maxMass);
  significance = output_significance [0];
  errsignificance = output_significance [2];
  return;
}

//__________________________________________________________________________

double* AliHFInvMassFitter::Significance(Double_t min, Double_t max) const {
  /// Return significance integral in a range
  ///
  double* background_info = new double [2];
  double* significance_info = new double [2];

    Double_t significance, errsignificance;
    Double_t background, errbackground;
  
    background_info = Background(min,max);
    background = background_info[0];
    errbackground = background_info[1];
  
    significance =  significance_info [0];
    errsignificance =  significance_info [1];
...
void AliHFInvMassFitter::ComputeSignificance(Double_t signal, Double_t  errsignal, Double_t  background, Double_t  errbackground, Double_t &significance,Double_t &errsignificance) const {
/// Return significance integral in a range
...
TH1F* AliHFInvMassFitter::SetTemplateReflections(const TH1 *h, TString opt,Double_t minRange,Double_t maxRange){
  ...
  //fHistoTemplRfl->GetXaxis()->SetRangeUser(1.72,2.01);
