#ifndef __PYJETTY_ALIHFINVMASSFITTER__HH
#define __PYJETTY_ALIHFINVMASSFITTER__HH
...
class AliHFInvMassFitter : public TNamed {
  ...
  void InvMassFitter(const TH1F *histoToFit, Double_t minvalue, Double_t maxvalue, Int_t fittypeb, Int_t fittypes);
  ...
  void     SetHistogramReflectionFit(const TH1F* histoReflToFit){
      if (fHistoTemplRfl) delete fHistoTemplRfl;
      fHistoTemplRfl=(TH1F*)histoReflToFit->Clone("fHistoTemplRfl");
      fHistoTemplRfl->SetDirectory(0);
    }
  ...
  void SetPolDegreeForBackgroundFit(Int_t deg){
    if(fTypeOfFit4Bkg!=6) std::cout<<"fTypeOfFit4Bkg should be set to 6 to use higher order polynomials\n"<<std::endl;
    fPolDegreeBkg=deg;
    SetNumberOfParams();
  }
  ...
  void SetBoundGaussianMean(Double_t mean, Double_t meanLowerLim, Double_t meanUpperLim){
      if(!(meanLowerLim < mean && mean < meanUpperLim)) std::cout<<"fMass limits are not set correctly\n"<<std::endl;
    SetInitialGaussianMean(mean); 
  ...
  void SetFixSecondGaussianSigma(Double_t sigma){
      if(fTypeOfFit4Sgn!=k2Gaus) std::cout<<"fTypeOfFit4Sgn should be set to k2Gaus to fix ratio between gaussians\n"<<std::endl;
    SetInitialSecondGaussianSigma(sigma);
    fFixedSigma2Gaus=kTRUE;
  }
  void SetFixFrac2Gaus(Double_t frac){
      if(fTypeOfFit4Sgn!=k2Gaus && fTypeOfFit4Sgn!=k2GausSigmaRatioPar) std::cout<<"fTypeOfFit4Sgn should be set to k2Gaus to fix relative integral of two gaussians\n"<<std::endl;
    SetInitialFrac2Gaus(frac);
    fFixedFrac2Gaus=kTRUE;
  }
  void SetFixRatio2GausSigma(Double_t sigmafrac){
      if(fTypeOfFit4Sgn!=k2GausSigmaRatioPar) std::cout<<"fTypeOfFit4Sgn should be set to k2GausSigmaRatioPar to fix ratio between gaussian sigmas\n"<<std::endl;
  ...
  double*     Signal(Double_t min,Double_t max) const;
  void Background(Double_t nOfSigma, Double_t &background,Double_t &errbackground) const;
  double* Background(Double_t min, Double_t max) const;
  ...
  double* Significance(Double_t min, Double_t max) const;
 ...
 private:
  ...
  TH1F*     fHistoRfl;        /// histogram with reflection template
  ...
  ClassDef(AliHFInvMassFitter,9)
    };/// class for invariant mass fit
