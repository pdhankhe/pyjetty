...
################################################################
class ProcessMC_dEEC_histograms(process_mc_base.ProcessMCBase):
  ...
  def __init__(self, input_file='', config_file='', output_dir='', save_tuples=1, debug_level=0, **kwargs):
    ...
    super(ProcessMC_dEEC_histograms, self).__init__(input_file, config_file, output_dir, save_tuples, debug_level, **kwargs)
    ...
    for observable in self.observable_list:
      ...
      for trk_thrd in self.obs_settings[observable]:
        ...
        if 'jet_pt' in observable:
          ...
          # fix event id, jet id bins
          name = 'h_JETINFO{}_Truth_R{}_{}'.format(observable, jetR, obs_label)
          id_bins = linbins(0,20,20)
          rc_bins = linbins(-1, 1, 200)
          dim = 10
          # title_truth = ["event_id:jet_id:jet_num_in_ev:jet_pt:total_num_const:num_const_aftercut:corr_rc:leading_q:subleading_q:total_num_baryons:num_baryons_aftercut:total_num_mesons:num_mesons_aftercut"]
          title_truth = ["jet_pt", "total_num_const","num_const_aftercut","corr_rc","leading_q","subleading_q","total_num_baryons","num_baryons_aftercut","total_num_mesons","num_mesons_aftercut"]
          binnings = (pt_bins, id_bins, id_bins, rc_bins, rc_bins, rc_bins, id_bins, id_bins, id_bins, id_bins)
          ...
          name = 'h_JETINFO{}_R{}_{}'.format(observable, jetR, obs_label)
          dim = 10
          title = ["jet_pt", "total_num_const","num_const_aftercut","corr_rc","leading_q","subleading_q","total_num_baryons","num_baryons_aftercut","total_num_mesons","num_mesons_aftercut"]
          binnings = (pt_bins, id_bins, id_bins, rc_bins, rc_bins, rc_bins, id_bins, id_bins, id_bins, id_bins)
        ...
        if "corr" in observable:
          ...
          self.fsparsepartonJetvalue = array.array( 'd', np.zeros(5)) #( 0, 0, 0, 0, 0 ))
  ...
  def create_corr_histograms(self, observable, ipoint, jetR, obs_label):
    ...
    if (observable == "corr_charge"):
      title_truth = ['p_{T,ch jet,truth}', 'R_{L,truth}_pt2040', 'R_{L,truth}_4060', 'R_{L,truth}_pt6080', 'charge_{truth}']
      title = ['p_{T,ch jet,det}', 'R_{L,det}_pt2040', 'R_{L,det}_pt4060', 'R_{L,det}_pt6080', 'charge_{det}']
  ...
  def fill_observable_histograms(self, hname, jet, jet_groomed_lund, jetR, obs_setting,
    ...
    for observable in self.observable_list:
      ...
      if 'jet_pt' in observable:
        ...
        if self.ENC_fastsim and (not 'Truth' in hname):
          ...
          jetinfo_filling = array.array('d', np.zeros(10))
          jetinfo_filling[0] = jet_pt
          jetinfo_filling[1] = len(constituents)
          jetinfo_filling[2] = nconst_jet
          jetinfo_filling[3] = leadq_subleadq
          jetinfo_filling[4] = leading_q
          jetinfo_filling[5] = subleading_q
          jetinfo_filling[6] = num_baryons_tot
          jetinfo_filling[7] = num_baryons_aftercut
          jetinfo_filling[8] = num_mesons_tot
          jetinfo_filling[9] = num_mesons_aftercut
          getattr(self, 'h_JETINFOjet_pt_Truth_R{}_{}'.format(jetR, obs_label)).Fill(jetinfo_filling)
    ...
    for index in range(new_corr.correlator(ipoint).rs().size()):
      for observable in self.observable_list:
        ...
        if (observable == "jet_ENC_RL" or observable == "jet_EEC_noweight_RL"):
          continue
        if (observable == "jet_pt"):
          continue
        if (observable == "corr_beg" or observable == "corr_rc" or observable == "corr_end"):
          continue
        ...
        # print("indices 1:",self.fsparsepartonJetvalue[1] )
        
        if "energyweights" in observable:
          self.fsparsepartonJetvalue[4] = new_corr.correlator(ipoint).weights()[index]
          # print("indices 2:",self.fsparsepartonJetvalue[2] )

        elif observable == 'corr_deltap':
          self.fsparsepartonJetvalue[4] = deltap_obs_corr.correlator(ipoint).rs()[index]
          # p1, p2 = self.mom_p1p2(new_corr, ipoint, c_select, index)
          # self.fsparsepartonJetvalue[4] = p1
          # self.fsparsepartonJetvalue[5] = p2
          # print("indices 3, 4, 5:",self.fsparsepartonJetvalue[3], self.fsparsepartonJetvalue[4], self.fsparsepartonJetvalue[5] )

        elif observable == 'corr_deltapt':
          self.fsparsepartonJetvalue[4] = deltapt_obs_corr.correlator(ipoint).rs()[index]
          # pt1, pt2 = self.transmom_p1p2(new_corr, ipoint, c_select, index)
          # self.fsparsepartonJetvalue[7] = pt1
          # self.fsparsepartonJetvalue[8] = pt2
          # print("indices 6, 7, 8:",self.fsparsepartonJetvalue[6], self.fsparsepartonJetvalue[7], self.fsparsepartonJetvalue[8] )
          
        elif observable == 'corr_deltapl':
          self.fsparsepartonJetvalue[4] = deltapl_obs_corr.correlator(ipoint).rs()[index]
          # pl1, pl2 = self.longmom_p1p2(new_corr, ipoint, c_select, index)
          # self.fsparsepartonJetvalue[10] = pl1
          # self.fsparsepartonJetvalue[11] = pl2
          # print("indices 9, 10, 11:",self.fsparsepartonJetvalue[9], self.fsparsepartonJetvalue[10], self.fsparsepartonJetvalue[11] )
          ...
          # self.fsparsepartonJetvalue[13] = q1
          # self.fsparsepartonJetvalue[14] = q2
          # print("indices 12, 13, 14:",self.fsparsepartonJetvalue[12], self.fsparsepartonJetvalue[13], self.fsparsepartonJetvalue[14] )
          ...
          # self.fparsepartonJetvalue[16] = pid1
          # self.fparsepartonJetvalue[17] = pid2
        ...
        #   # print("IN CORR_END WRITING", self.fsparsepartonJetvalue)
        #   # print("IN CORR_END WRITING", self.fsparsepartonJetvalue[0])
        #   #TODO: again, don't do det level until you know what you're doing
        #   # if self.ENC_fastsim and (not 'Truth' in hname):
        #   #   getattr(self, 'tn_pairlevel_R{}_{}'.format(jetR, obs_label)).Fill(np.array(list(self.fsparsepartonJetvalue), dtype='float32'))
        #   # else:
        #   # print("IN END!!!!", self.fsparsepartonJetvalue )
        #   getattr(self, 'h_pairlevel_Truth_R{}_{}'.format(jetR, obs_label)).Fill(np.array(list(self.fsparsepartonJetvalue), dtype='float32'))
...
if __name__ == '__main__':
  ...
  analysis = ProcessMC_dEEC_histograms(input_file=args.inputFile, config_file=args.configFile, output_dir=args.outputDir, save_tuples=args.saveTuples)
  analysis.process_mc()