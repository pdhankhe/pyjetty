...
def FormatHist(hist, norm_factor, color):
    hist.Scale(1/norm_factor, "width")
    hist.SetLineColor(color)
    hist.SetLineWidth(2)

def GetCABHist(hist_AA, hist_BB, hist_AB, name="hist_correlation"):
    """
...
def main():
    ...
    # Jet Definitions
    jet_R = 0.4
    jet_def_ca = fj.JetDefinition(fj.cambridge_algorithm, jet_R)
    ...
    for i, target_jetpt in enumerate(target_jet_pts):

        # Load the data
        # path = f"/global/cfs/cdirs/alice/alicepro/hiccup/rstorage/alice/AnalysisResults/blianggi/jse/pythia_otf/{target_jet_pts[i]}gev/JetsForAnalysis.parquet"
        path = f"/global/cfs/cdirs/alice/alicepro/hiccup/rstorage/alice/AnalysisResults/blianggi/jse/pythia_otf/51506550/{target_jet_pts[i]}gev/FilteredJetsForAnalysisCombined.parquet"
        # path = f"/global/cfs/cdirs/alice/alicepro/hiccup/rstorage/alice/AnalysisResults/blianggi/jse/pythia_otf/51506550/{target_jet_pts[i]}gev/{n+1}/JetsForAnalysis.parquet"
        ...
        for z_cut in z_cuts:
            ...
            hist_full = ROOT.TH1D(f"hist_full_jetpt{target_jetpt}_zcut{z_cut}", "EEC; R_{L}", nbins, log_bins)

            hist_AA_wwjetpt = ROOT.TH1D(f"hist_AA_jetpt{target_jetpt}_zcut{z_cut}_wwjetpt", "AxA; R_{L}", nbins, log_bins)
            hist_BB_wwjetpt = ROOT.TH1D(f"hist_BB_jetpt{target_jetpt}_zcut{z_cut}_wwjetpt", "BxB", nbins, log_bins)
            hist_AB_wwjetpt = ROOT.TH1D(f"hist_AB_jetpt{target_jetpt}_zcut{z_cut}_wwjetpt", "AxB", nbins, log_bins)

            hist_AA_wwradpt = ROOT.TH1D(f"hist_AA_jetpt{target_jetpt}_zcut{z_cut}_wwradpt", "AxA; R_{L}", nbins, log_bins)
            hist_BB_wwradpt = ROOT.TH1D(f"hist_BB_jetpt{target_jetpt}_zcut{z_cut}_wwradpt", "BxB", nbins, log_bins)
            hist_AB_wwradpt = ROOT.TH1D(f"hist_AB_jetpt{target_jetpt}_zcut{z_cut}_wwradpt", "AxB", nbins, log_bins)
            ...
            hist_radiatorpt = ROOT.TH1D(f"radiator_pt_jetpt{target_jetpt}_zcut{z_cut}", "radiator p_{T}; p_{T,radiator}", target_jetpt, radpt_bins)
            hist_radiatorkt = ROOT.TH1D(f"ln_radiator_kt_jetpt{target_jetpt}_zcut{z_cut}", "ln radiator k_{T}; ln(k_{T,radiator})", nbins, radkt_bins)

            num_jets = 0.
            num_jets_passed_SD = 0.


            # for jet_id, jet_constituents in grouped_jets:
            for (event_idx, jet_id), jet_constituents in grouped_jets:
                ...
                cs = fj.ClusterSequence(pj_particles, jet_def_ca)
                ...
                lund_gen = fjcontrib.LundGenerator(jet_def_ca) #lp.LundGenerator(jet_def)
                ...
                # sd_info = {"jet_id": jet_id, "z": None, "theta": None, "kt": None, "sj1_pt": None, "sj2_pt": None, "n_constituents_1": None, "n_constituents_2": None}
                ...
                for d in lund_plane_elements:
                    ...
                    if d.z() > z_cut:
                        ...
                        # Get the constituents of each subjet - this returns a list of PseudoJets for the particles in each prong
                        constituents_1 = subjet_a.constituents()
                        constituents_2 = subjet_b.constituents()

                        # Store the info
                        # sd_info.update({ "z": d.z(),
                        #                  "theta": d.Delta(), # Angular separation
                        #                  "kt": d.kt(),       # Relative transverse momentum
                        #                  "sj1_pt": subjet_a.pt(),
                        #                  "sj2_pt": subjet_b.pt(),
                        #                  "n_constituents_1": len(constituents_1),
                        #                  "n_constituents_2": len(constituents_2) })
                        
                        # Example: Accessing the pt of the first particle in subjet 1
                        # first_particle_pt = constituents_1[0].pt()
            ...
            FormatHist(hist_full, num_jets_passed_SD, ROOT.kBlack)

            FormatHist(hist_AA_wwjetpt, num_jets_passed_SD, ROOT.kBlue)
            FormatHist(hist_BB_wwjetpt, num_jets_passed_SD, ROOT.kOrange+7)
            FormatHist(hist_AB_wwjetpt, num_jets_passed_SD, ROOT.kGreen+2)

            FormatHist(hist_AA_wwradpt, num_jets_passed_SD, ROOT.kBlue)
            FormatHist(hist_BB_wwradpt, num_jets_passed_SD, ROOT.kOrange+7)
            FormatHist(hist_AB_wwradpt, num_jets_passed_SD, ROOT.kGreen+2)


            # Add a Legend
            legend = ROOT.TLegend(0.7, 0.5, 0.9, 0.65)
            legend.AddEntry(hist_full, "all jets that passed SD", "l")
            ...
            legend.Draw()
            canvas_wwjetpt.SaveAs(f"/global/cfs/cdirs/alice/blianggi/mypyjetty/storage/jse/plots/subjet_eec_comparison_jetpt{target_jetpt}_R0.4_sd{z_cut}_wwjetpt.pdf")
            ...
            legend.Draw()
            canvas_wwradpt.SaveAs(f"/global/cfs/cdirs/alice/blianggi/mypyjetty/storage/jse/plots/subjet_eec_comparison_jetpt{target_jetpt}_R0.4_sd{z_cut}_wwradpt.pdf")

            
            # Calulate C_AB
            can_CAB.cd()
            # Usage in your main script:
            hist_CAB_wwjetpt = GetCABHist(hist_AA_wwjetpt, hist_BB_wwjetpt, hist_AB_wwjetpt, "CAB_wwjetpt")
            hist_CAB_wwradpt = GetCABHist(hist_AA_wwradpt, hist_BB_wwradpt, hist_AB_wwradpt, "CAB_wwradpt")

            FormatHist(hist_CAB_wwjetpt, 1, ROOT.kPink+10)
            FormatHist(hist_CAB_wwradpt, 1, ROOT.kViolet+7)
            ...
            legend_CAB.Draw()
            can_CAB.SaveAs(f"/global/cfs/cdirs/alice/blianggi/mypyjetty/storage/jse/plots/CAB_jetpt{target_jetpt}_R0.4_sd{z_cut}.pdf")

            # Plot jet level info
            can_radpt.cd()
            hist_radiatorpt.Draw()
            can_radpt.SaveAs(f"/global/cfs/cdirs/alice/blianggi/mypyjetty/storage/jse/plots/radiator_pt_jetpt{target_jetpt}_R0.4_sd{z_cut}.pdf")
            
            can_radkt.cd()
            hist_radiatorkt.Draw()
            can_radkt.SaveAs(f"/global/cfs/cdirs/alice/blianggi/mypyjetty/storage/jse/plots/radiator_kt_jetpt{target_jetpt}_R0.4_sd{z_cut}.pdf")
