...
ROOT_OUTFILE=${OUTDIR}/AnalysisResults_ungroomedbins.root
python ${SCRIPT_PROCESS_JETS} ${PARQUET_OUTFILE} ${ROOT_OUTFILE} --zcuts 0.1 0.2 --no-maxkt --add-noweight --binning ungroomed

ROOT_OUTFILE_GR=${OUTDIR}/AnalysisResults_groomedbins.root
python ${SCRIPT_PROCESS_JETS} ${PARQUET_OUTFILE} ${ROOT_OUTFILE_GR} --zcuts 0.1 0.2 --no-maxkt --add-noweight --binning groomed
...
mv /global/cfs/projectdirs/alice/alicepro/hiccup/rstorage/alice/AnalysisResults/blianggi/jse/slurm-${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out /global/cfs/projectdirs/alice/alicepro/hiccup/rstorage/alice/AnalysisResults/blianggi/jse/data/${SLURM_ARRAY_JOB_ID}/slurm-output/${SLURM_ARRAY_JOB_ID}
