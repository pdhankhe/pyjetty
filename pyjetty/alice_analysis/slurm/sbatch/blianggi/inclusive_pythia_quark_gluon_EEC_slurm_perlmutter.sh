...
PTHAT_BINS=(8 16 32 48) # aiming for 80% target jet pt
...
SCRIPT="${BASEDIR}/pyjetty/pyjetty/alice_analysis/process/user/blianggi/pythia_quark_gluon_EEC_inclusive.py"
CONFIG="${BASEDIR}/pyjetty/pyjetty/alice_analysis/config/ENC/pp/configcuts_ptbin.yaml"

if $USE_PTHAT_MAX; then
    echo "pipenv run python $SCRIPT -c $CONFIG --output-dir $OUTDIR --user-seed $SEED --py-pthatmin $PTHAT_MIN --py-ecm $ECM --nev $NEV_PER_JOB --pythiaopts HardQCD:all=on,TimeShower:pTmin=0.2,PhaseSpace:pTHatMax=$PTHAT_MAX --weightON 1"
    pipenv run python $SCRIPT -c $CONFIG --output-dir $OUTDIR --user-seed $SEED \
        --py-pthatmin $PTHAT_MIN --py-ecm $ECM --nev $NEV_PER_JOB \
        --pythiaopts HardQCD:all=on,TimeShower:pTmin=0.2,PhaseSpace:pTHatMax=$PTHAT_MAX --weightON 1
else
    echo "pipenv run python $SCRIPT -c $CONFIG --output-dir $OUTDIR --user-seed $SEED --py-pthatmin $PTHAT_MIN --py-ecm $ECM --nev $NEV_PER_JOB --pythiaopts HardQCD:all=on,TimeShower:pTmin=0.2 --weightON 1"
