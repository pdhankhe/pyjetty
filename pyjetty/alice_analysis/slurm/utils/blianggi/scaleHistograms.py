...
def ScaleAllHistograms(obj, scaleFactor, f, verbose, bRemoveOutliers=False, limit=2, nBinsThreshold=4, pTHardBin=0, EndPtHardBin=20, taskName=""):
  ...
  elif obj.InheritsFrom(ROOT.RooUnfoldResponse.Class()):
    # TH1 *hMeasured = obj.Hmeasured()
    if verbose:
      print('Roounfold object - no Sumw2 on {}'.format(obj.GetName()))
  else:
  ...
  if obj.InheritsFrom(ROOT.TProfile.Class()):
    ...
    if verbose:
      print("Roounfold Response %s cannot be scaled..." % obj.GetName())
  else:
    if verbose:
