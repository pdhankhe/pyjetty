...
if $USE_PTHAT_MAX; then
    ...
    pipenv run python $SCRIPT -c $CONFIG --output-dir $OUTDIR --user-seed $SEED \
        ...
        --pythiaopts HardQCD:all=on,TimeShower:pTmin=0.2,PhaseSpace:pTHatMax=$PTHAT_MAX --replaceKP 1 --chinitscat 3 --rigidcone 1.0
else
    ...
    pipenv run python $SCRIPT -c $CONFIG --output-dir $OUTDIR \
        ...
        --pythiaopts HardQCD:all=on,TimeShower:pTmin=0.2 --replaceKP 1 --chinitscat 3 --rigidcone 1.0
fi