...
class RunAnalysisAng(run_analysis.RunAnalysis):
  ...
  def rawJetSpectra(self, sparse, jetR):
    ...
    for beta in self.beta_list:
        ...
        for ibin2 in range(len(self.fptbinsJetFinalA)-1):
            ...
            self.fbinsJetAngularityA = array('d', (0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55,0.6, 0.65, 0.7, 0.75, 0.8))
            hAngdist = hang_corr.Clone("hAngdist" + suffix_jet +"_"+str(beta))
            hRebinAngdist = hAngdist.Rebin(16, "hRebinAngdist", self.fbinsJetAngularityA)
        ...
        hangvsjetpt_feedsub.Scale(1 / hangvsjetpt_feedsub.Integral(), "width")
  ...
  def feeddown(self, obs_axis, jetR, beta ):
    ...
    roounfold_filename = os.path.join(self.output_dir_fd, 'fRoounfold.root')
    ...
    fout = ROOT.TFile("AnalysisResults_outputfd_1.root", 'recreate')
