...
class RunAnalysisAng(run_analysis.RunAnalysis):
  ...
  def fill_matching_histograms(self):
    ...
    sparse = self.file_simulation.Get("hsparsejet")
    
    
    sparse_JES_prompt = sparse.Clone("sparse_JES_prompt")
    sparse_JES_prompt.GetAxis(0).SetRangeUser(self.jetrange[0], self.jetrange[1])
    sparse_JES_prompt.GetAxis(1).SetRangeUser(self.jetrange[0], self.jetrange[1])
    sparse_JES_prompt.GetAxis(5).SetRangeUser(1, 2) #prompt
    sparse_JES_prompt.GetAxis(6).SetRangeUser(0, 1) #non prompt
    sparse_JES_prompt.GetAxis(7).SetRangeUser(0,1)  #reflection
    
    jet_pt_prompt_info = sparse_JES_prompt.Projection(0,1)
  ...
  def fill_response_histograms(self):
    ...
    label = "R%s_%s" % (str(0.4), str(1))
    
    for ipt in range(self.fptbinsDN):
        ptbin = (self.fptbinsDA[ipt]+self.fptbinsDA[ipt+1]) / 2.
        sparse_response_prompt = sparse.Clone("sparse_JES_prompt")
        sparse_response_prompt.GetAxis(0).SetRangeUser(2, 60)
        sparse_response_prompt.GetAxis(1).SetRangeUser(2, 60)
        sparse_response_prompt.GetAxis(2).SetRangeUser(self.fptbinsDA[ipt], self.fptbinsDA[ipt + 1]) #dmesonpt
        sparse_response_prompt.GetAxis(5).SetRangeUser(1, 2) #prompt
        sparse_response_prompt.GetAxis(6).SetRangeUser(0, 1) #non prompt
        sparse_response_prompt.GetAxis(7).SetRangeUser(0,1)  #reflection
        
        prompt_response = sparse_response_prompt.Projection(ndim, dim, "E")
    ...
    for ipt in range(self.fptbinsDN):
        ptbin = (self.fptbinsDA[ipt]+self.fptbinsDA[ipt+1]) / 2.
        sparse_response_nonprompt = sparse.Clone("sparse_JES_prompt")
        sparse_response_nonprompt.GetAxis(0).SetRangeUser(2, 60)
        sparse_response_nonprompt.GetAxis(1).SetRangeUser(2, 60)
        sparse_response_nonprompt.GetAxis(2).SetRangeUser(self.fptbinsDA[ipt], self.fptbinsDA[ipt + 1]) #dmesonpt
        sparse_response_nonprompt.GetAxis(5).SetRangeUser(0, 1) #prompt
        sparse_response_nonprompt.GetAxis(6).SetRangeUser(1, 2) #non prompt
        sparse_response_nonprompt.GetAxis(7).SetRangeUser(0,1)  #reflection
        
        nonprompt_response = sparse_response_nonprompt.Projection(ndim, dim, "E")
