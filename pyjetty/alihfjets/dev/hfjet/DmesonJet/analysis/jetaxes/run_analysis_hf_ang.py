...
class RunAnalysisAng(run_analysis.RunAnalysis):
  ...
  def hf_jet_shape_extraction(self):
    ...
    fout = ROOT.TFile("AnalysisResults_std_output.root", 'recreate')
    sparse = file_data.Get("hsparsejet")
    #okSignalExt = self.rawJetSpectra(sparse, jetR, fout, 2, 3, False)
    
    foutwta = ROOT.TFile("AnalysisResults_WTA_output.root", 'recreate')
    sparse = file_data.Get("hsparsejetWTA")
    #okSignalExt = self.rawJetSpectra(sparse, jetR,foutwta, 3, 4, False)
    
    foutSD = ROOT.TFile("AnalysisResults_SD_z_output.root", 'recreate')
    sparse = file_data.Get("hsparsejetSD")
    okSignalExt = self.rawJetSpectra(sparse, jetR,foutSD, 3, 7, True)
    if not okSignalExt:
  ...
  def rawJetSpectra(self, sparse, jetR, fout, invmass, obs_axis, bool):
    
    norm = [36986.63, 12897.97, 5250.47, 1118.41]
    ...
    for beta in self.beta_list:
        ...
        for ibin2 in range(len(self.fptbinsJetFinalA)-1):
            ...
            sparsehadron.GetAxis(4).SetRangeUser(0,0.75);
            #select jet pt bins
            ...
            h_invmass_shape_Dmesonpt = sparsehadron.Projection(invmass, obs_axis, invmass-1)
            ...
            for ipt in range(self.fptbinsDN):
              ...
              hinvmass = h_invmass_shape_Dmesonpt.ProjectionX("hinvmass"+suffix, h_invmass_shape_Dmesonpt.GetYaxis().FindBin(0.00001),
              ...
              input_data_invmass_list.append(h)
            ...
            hRebinAngdist = hAngdist.Rebin(len(self.fobs_bins)-1, "hRebinAngdist_JetPt%d_R%s_%s" % (self.fptbinsJetFinalA[ibin2],jetR, beta), obs_bin_array)
            if not bool:
                print("the values of jets", str(hRebinAngdist.Integral()))
                hRebinAngdist.Scale(1/hRebinAngdist.Integral(),"width")
                
            else:
                hRebinAngdist.Scale(1/norm[ibin2],"width")
            hRebinAngdist.GetXaxis().SetTitle("#lambda^{k=1}_{#alpha=1}")
            hRebinAngdist.GetYaxis().SetTitle("1/#it{N}_{jet} d#it{N}/d#lambda")
            name = "h_radial_std_JetPt%d_R%s_%s" % (self.fptbinsJetFinalA[ibin2],jetR, beta)
