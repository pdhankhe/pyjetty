    ...
    for beta in self.beta_list:
        ...
        for ibin2 in range(len(self.fptbinsJetFinalA)-1):
            ...
            for ipt in range(self.fptbinsDN):
              ...
              eff_value_err = self.efficiency_prompt.GetBinError(self.efficiency_prompt.GetXaxis().FindBin(ptbin))

              hang_sig_bkgsub_effcorr.Scale(1/eff_value)
              #for ibinx in range(hang_sig_bkgsub_effcorr.GetNbinsX()):
              #  for ibiny in range(hang_sig_bkgsub_effcorr.GetNbinsY()):
              #      content = hang_sig_bkgsub_effcorr.GetBinContent(ibinx+1,ibiny+1)
              #      content_error = hang_sig_bkgsub_effcorr.GetBinError(ibinx+1,ibiny+1)
              #      hang_sig_bkgsub_effcorr.SetBinContent(ibinx+1,ibiny+1, content/eff_value)
              #      if content > 0:
              #          error = ErrorCalUnCorr(content, eff_value, content_error, eff_value_err)
              #          hang_sig_bkgsub_effcorr.SetBinError(ibinx+1,ibiny+1,error)
            ...
            test1 = self.histutils.rebin_th2(hrawjetptspectrum,"hang_sb_sub"+suffix_jet, jet_pt_bin_array, len(self.fptbinsJetFinalA)-1, obs_bin_array, len(self.fobs_bins)-1)
            test1.Sumw2()
            
            
            test2 = self.histutils.rebin_th2(hang_corr,"hang_sb_sub"+suffix_jet, jet_pt_bin_array, len(self.fptbinsJetFinalA)-1, obs_bin_array, len(self.fobs_bins)-1)
            test2.Sumw2()
            
            hang_sb_sub_rebinned_list.append(test1)
            hang_eff_corr_rebinned_list.append(test2)
     

            hangvsjetpt_sb_sub_projy2 = hang_sb_sub_rebinned_list[ibin2].ProjectionY()
            hangvsjetpt_sb_sub_projy2.Scale(1/hangvsjetpt_sb_sub_projy2.Integral(),"width")
            hangvsjetpt_sb_sub_projy2.GetYaxis().SetTitle("1/N_{jet} dN/d#lambda")
            hangvsjetpt_sb_sub_projy2.GetXaxis().SetTitle("#lambda")
            
            cshapevsjetpt1 = ROOT.TCanvas("cshapevsjetpt1", "cshapevsjetpt1")
            hangvsjetpt_sb_sub_projy2.Draw()
            cshapevsjetpt1.SaveAs("hangvsjetpt_sb_sub_{}_{}.pdf".format(self.fptbinsJetFinalA[ibin2],self.fptbinsJetFinalA[ibin2+1]))
            
            hang_corr_projy2 = hang_eff_corr_rebinned_list[ibin2].ProjectionY()
            hang_corr_projy2.Scale(1/hang_corr_projy2.Integral(),"width")
            hang_corr_projy2.GetYaxis().SetTitle("1/N_{jet} dN/d#lambda")
            hang_corr_projy2.GetXaxis().SetTitle("#lambda")
            
            cshapevsjetpt2 = ROOT.TCanvas("cshapevsjetpt2", "cshapevsjetpt2")
            hang_corr_projy2.Draw()
            cshapevsjetpt2.SaveAs("hangvsjetpt_sb_sub_eff_{}_{}.pdf".format(self.fptbinsJetFinalA[ibin2],self.fptbinsJetFinalA[ibin2+1]))
            
            
            hangvsjetpt_sb_sub_content_to_error = hangvsjetpt_sb_sub_projy2.Clone("hangvsjetpt_sb_sub_content_to_error"+suffix_jet)
            hang_corr_content_to_error = hang_corr_projy2.Clone("hangvsjetpt_sb_sub_content_to_error"+suffix_jet)
            
            for ibinx in range(len(self.fobs_bins)-1):
                    if (hangvsjetpt_sb_sub_content_to_error.GetBinContent(ibinx+1)) >0:
                        hangvsjetpt_sb_sub_content_to_error.SetBinContent(ibinx+1,hangvsjetpt_sb_sub_content_to_error.GetBinError(ibinx+1)/hangvsjetpt_sb_sub_content_to_error.GetBinContent(ibinx+1))
                        hangvsjetpt_sb_sub_content_to_error.SetBinError(ibinx+1,0)
                    if (hang_corr_content_to_error.GetBinContent(ibinx+1)) >0:
                        hang_corr_content_to_error.SetBinContent(ibinx+1,hang_corr_content_to_error.GetBinError(ibinx+1)/hang_corr_content_to_error.GetBinContent(ibinx+1))
                        hang_corr_content_to_error.SetBinError(ibinx+1,0)
                   
            cshapevsjetpt5 = ROOT.TCanvas("cshapevsjetpt5", "cshapevsjetpt5")
            process_canvas(cshapevsjetpt5)
            hangvsjetpt_sb_sub_content_to_error.GetYaxis().SetTitle("Stat_error/content")
            hangvsjetpt_sb_sub_content_to_error.GetXaxis().SetTitle("#lambda")
            hangvsjetpt_sb_sub_content_to_error.Draw()
            cshapevsjetpt5.SaveAs("hangvsjetpt_sb_sub_content_to_error_{}_{}.pdf".format(self.fptbinsJetFinalA[ibin2],self.fptbinsJetFinalA[ibin2+1]))
            
            cshapevsjetpt6 = ROOT.TCanvas("cshapevsjetpt6", "cshapevsjetpt6")
            process_canvas(cshapevsjetpt2)
            hang_corr_content_to_error.GetYaxis().SetTitle("Stat_error/content")
            hang_corr_content_to_error.GetXaxis().SetTitle("#lambda")
            hang_corr_content_to_error.Draw()
            cshapevsjetpt6.SaveAs("hangvsjetpt_sb_sub_eff_content_to_error_{}_{}.pdf".format(self.fptbinsJetFinalA[ibin2],self.fptbinsJetFinalA[ibin2+1]))
            
            
         
            
            
    for ibinx in range(len(self.fptbinsJetFinalA)-1):
        for ibiny in range(len(self.fobs_bins)-1):
            hangvsjetpt_sb_sub.SetBinContent(ibinx+1,ibiny+1,hang_sb_sub_rebinned_list[ibin2].GetBinContent(ibinx+1,ibiny+1))
            hangvsjetpt_sb_sub.SetBinError(ibinx+1,ibiny+1,hang_sb_sub_rebinned_list[ibin2].GetBinError(ibinx+1,ibiny+1))
            hangvsjetpt_eff_corr.SetBinContent(ibinx+1,ibiny+1,hang_eff_corr_rebinned_list[ibin2].GetBinContent(ibinx+1,ibiny+1))
            hangvsjetpt_eff_corr.SetBinError(ibinx+1,ibiny+1,hang_eff_corr_rebinned_list[ibin2].GetBinError(ibinx+1,ibiny+1))
             
             
    cshapevsjetpt3 = ROOT.TCanvas("cshapevsjetpt3", "cshapevsjetpt3")
    process_canvas(cshapevsjetpt3)
    hangvsjetpt_sb_sub.Draw("text")
    cshapevsjetpt3.SaveAs("hangvsjet_sb_sub_stats.pdf")
    
    cshapevsjetpt4 = ROOT.TCanvas("cshapevsjetpt4", "cshapevsjetpt2")
    process_canvas(cshapevsjetpt4)
    hangvsjetpt_eff_corr.Draw("text")
    cshapevsjetpt4.SaveAs("hangvsjet_eff_corr_stats.pdf")
