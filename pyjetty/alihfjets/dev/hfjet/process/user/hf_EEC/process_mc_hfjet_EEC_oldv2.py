...
class HFAnalysisInvMass(hfdio.HFAnalysis):
	def __init__(self, **kwargs):
		...
		self.histo_particle_fd.Sumw2()
        	...
        	# add for EEC
		obs_bins_mass = np.concatenate((np.linspace(0, 0.9, 10), np.linspace(1, 9.8, 45), np.linspace(10, 14.5, 10), np.linspace(15, 19, 5), np.linspace(20, 60, 9)))
		...
		self.name_nonprompt='THnSparse_prompt_signal_ungroomedjetmass'
		self.fsparsejet_nonprompt_ungroomedjetmass=ROOT.THnSparseD(self.name_prompt,"Jet_gen_pt;Jet_reco_pt;D_pt_gen;D_pt_rec;invmass;ungroomedjetmass_gen;ungroomedjetmass_reco",dim,nbins_array,xmin_array,xmax_array)
		self.fsparsejet_nonprompt_ungroomedjetmass.Sumw2()
		self.fsparse_nonprompt_ungroomedjetmass_value=array('d',(0,0,0,0,0,0,0))
        
        
		self.name_prompt='THnSparse_nonprompt_signal_groomedjetmass'
		self.fsparsejet_nonprompt_groomedjetmass=ROOT.THnSparseD(self.name_prompt,"Jet_gen_pt;Jet_reco_pt;D_pt_gen;D_pt_rec;invmass;groomedjetmass_gen;groomedjetmass_reco",dim,nbins_array,xmin_array,xmax_array)
			...
			if delta_jet_R<(0.6*0.4):
				if delta_cand_R<(0.1):
					...
					if cand_type_prompt==1:
						...
						self.fsparsejet_prompt_groomedjetmass.Fill(self.fsparse_nonprompt_groomedjetmass_value)
