...
class HFAnalysisInvMass(hfdio.HFAnalysis, process_base.ProcessBase):
    ...
    def initialize_user_output_objects(self):
        ...
        title = [ '#it{p}_{T}^{ch jet}', '#it{p}_{T}^{D^{0}}', 'Invmass', '#it{R}_{L}', '#it{R}_{L} w n_{const}>2']
        ...
        dim = 5
        nbins  = [len(pt_bins_jet)-1, len(pt_bins_dmeson)-1, len(dmeson_mass_bin)-1, len(RL_bins)-1, len(RL_bins)-1]
        min_li = [pt_bins_jet[0],     pt_bins_dmeson[0],     dmeson_mass_bin[0],     RL_bins[0],     RL_bins[0]]
        max_li = [pt_bins_jet[-1],    pt_bins_dmeson[-1],    dmeson_mass_bin[-1],    RL_bins[-1],    RL_bins[-1]]
    ...
    def event_by_event_D_jet_reconstruction(self, df):
        ...
        for id0, d0 in enumerate(djmm.Ds):
            ...
            if len(djets) > 0:
                ...
                print("num constituents", len(constituents))
                ...
                # EEC with n_const > 2 (needed to make at least one pair)
                if len(constituents) > 2:
                    x_array = array( 'd', ( j.perp(), dcand[0].perp(), dcand[0].m(), j.delta_R(dcand[0]), j.delta_R(dcand[0])))
                ...
                print("num correlations", new_corr.correlator(2).rs().size())
                for index in range(new_corr.correlator(2).rs().size()):
#                    x_EEC_array = array( 'd', ( j.perp(), dcand[0].perp(), dcand[0].m(), new_corr.correlator(2).rs()[index], new_corr.correlator(2).rs()[index]))
#                    getattr(self, 'hsparse_R{}_{}'.format(self.JetR, self.observable_list[1])).Fill(x_EEC_array, new_corr.correlator(2).weights()[index])
