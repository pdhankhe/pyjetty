...
void make_qg_plots_noTHnSparse_write() {
...
//    gROOT->SetBatch(); //prevents plots from showing up
    ...
    f = new TFile(infile_noD0replacement_charmdecaysON, "READ");
    // TString add_name = "";
    ...
    std::string outfile = outdir + "AnalysisResultsFinal_allpartonjets_charmdecaysON.root"; 
    ...
    for (std::string jetR : jetR_list) {
        ...
        for (int i = 0; i < n_bins; i++) {
            ...
            TH1* hi_proj = (TH1*) hi2D->ProjectionY();
    ...
    //         double obs_bins[] = {1.00000000e-04, 1.25892541e-04, 1.58489319e-04, 1.99526231e-04,
            ...
            TH1* hc = (TH1*) hc_proj->Clone((hc_name + "_pt" + std::to_string(pt_min) + "-" + std::to_string(pt_max)).c_str());
            TH1* hl = (TH1*) hl_proj->Clone((hl_name + "_pt" + std::to_string(pt_min) + "-" + std::to_string(pt_max)).c_str());
            TH1* hg = (TH1*) hg_proj->Clone((hg_name + "_pt" + std::to_string(pt_min) + "-" + std::to_string(pt_max)).c_str());
            TH1* hi = (TH1*) hi_proj->Clone((hi_name + "_pt" + std::to_string(pt_min) + "-" + std::to_string(pt_max)).c_str());
            ...
            double numjets_inclusive = hi1D_jet->Integral(); //GetEntries();
            ...
            FormatHist(l, hl, label1, markercolor1, markerstyle1);
            FormatHist(l, hg, label1, markercolor1, markerstyle1);
            FormatHist(l, hi, label1, markercolor1, markerstyle1);
            l->AddEntry("NULL","          D* decays off","h");
            hl->Draw("L same");
            
            
            double hl_top_binpos = findTopOfCurve(hl);
            drawVertLine(hl->GetBinCenter(hl_top_binpos), 0, hl->GetBinContent(hl_top_binpos), markercolor1, 1)->Draw();
            ...
            f_out->cd();
            hc->Write(); // charm jets are empty.... idk why
            hl->Write();
            hg->Write();
            hi->Write();
