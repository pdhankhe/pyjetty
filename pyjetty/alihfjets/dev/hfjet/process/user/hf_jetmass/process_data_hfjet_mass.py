...
class HFAnalysisInvMass(hfdio.HFAnalysis):
	def __init__(self, **kwargs):
		...
		title = [ '#it{p}_{T}^{ch jet}', '#it{p}_{T}^{D^{0}}', 'Invmass', '#it{m}_{jet}']
		...
		dim = 4
		nbins  = [len(pt_bins_jet)-1, len(pt_bins_dmeson)-1, len(dmeson_mass_bin)-1, len(obs_bins_mass)-1]
		min_li = [pt_bins_jet[0],     pt_bins_dmeson[0],     dmeson_mass_bin[0],     obs_bins_mass[0]    ]
		max_li = [pt_bins_jet[-1],    pt_bins_dmeson[-1],    dmeson_mass_bin[-1],    obs_bins_mass[-1]   ]
		nbins = (nbins)
		xmin = (min_li)
		xmax = (max_li)
		...
		self.fsparseJet = ROOT.THnSparseD("hsparsejet","hsparsejet; Jet_pt; D_pt; D_m; jetmass", dim,  nbins_array, xmin_array, xmax_array)
		self.fsparseJetvalue = array( 'd', ( 0, 0, 0, 0))
		self.fsparseJet.Sumw2()
		
		self.fsparseJetSD = ROOT.THnSparseD("hsparsejet_groomed","hsparsejet_groomed; Jet_pt; D_pt; D_m; groomed_jetmass", dim, nbins_array, xmin_array, xmax_array)
		self.fsparseJetSDvalue = array( 'd', ( 0, 0, 0, 0))
	...
	def analysis(self, df):
		...
		for id0, d0 in enumerate(djmm.Ds):
			...
			if len(djets) > 0:
				...
				self.fsparseJetSDvalue[3] = jetmass
