...
//    gROOT->SetBatch(); //prevents plots from showing up
    ...
    Double_t colors[16] = {kRed, kGreen, kBlue, kRed+1, kGreen+1, kBlue+1, kRed+2, kGreen+2, kBlue+2, kRed+3, kGreen+3, kBlue+3, kOrange+1, kViolet+1, kYellow+1, kCyan+1};


    // File containing quark vs gluon histograms

     //FOR WHEN WEIGHTED/UNWEIGHTED IN SAME FILE
    const char infile_charmOFF[] = "/global/cfs/projectdirs/alice/alicepro/hiccup/rstorage/alice/AnalysisResults/blianggi/jetaxis/13028172/AnalysisResultsFinal.root"; //perlmutter
    // const char infile_charmON[] = "/global/cfs/projectdirs/alice/alicepro/hiccup/rstorage/alice/AnalysisResults/blianggi/jetaxis/11731473/AnalysisResultsFinal.root"; //perlmutter 

    bool include_gluon = true; //true = draw gluon, false = do not draw gluon
    bool switchON = true; //true = charm ON, false = charm OFF
    TFile* f;
    ...
    std::string charmswitchstring = switchON ? "_charmdecaysON" : "_charmdecaysOFF";
    std::string add_name = charmswitchstring + quarkstring + ".pdf"; 
...
//            for (std::string grooming : grooming_list) {
        ...
        const std::string hc_name = "h_jet_axis_JetPt_charm_R" + jetR + "_['STD-D']";
        const std::string hl_name = "h_jet_axis_JetPt_light_R" + jetR + "_['STD-D']";
        const std::string hg_name = "h_jet_axis_JetPt_gluon_R" + jetR + "_['STD-D']";
        const std::string hc_jet_name = "h_JetPt_charm_R" + jetR + "_['STD-D']_jetlevel";
        const std::string hl_jet_name = "h_JetPt_light_R" + jetR + "_['STD-D']_jetlevel";
        const std::string hg_jet_name = "h_JetPt_gluon_R" + jetR + "_['STD-D']_jetlevel";
        ...
        const int pt_bins[] = { 5, 7, 10, 20, 50 }; //{ 10, 20, 40 };
        const int n_bins = 4; //2;
...
//            hq->SetMinimum(1e-3);
            double maxy = include_gluon ? hg->GetMaximum() : 0;
            ...
            FormatHist(l, hc, "Charm-initiated jets", colors[0], 20);
            FormatHist(l, hl, "Light-initiated jets", colors[1], 22);


            // // Fix the x-axis label
            hc->GetXaxis()->SetTitle("#DeltaR");
            hc->GetYaxis()->SetTitle("#frac{1}{#it{N}_{jet}} #frac{d#it{N}}{d#DeltaR}");
            ...
            if (include_gluon) {
                FormatHist(l, hg, "Gluon-initiated jets", colors[2], 33);
                hg->Draw("L same");
            }  
