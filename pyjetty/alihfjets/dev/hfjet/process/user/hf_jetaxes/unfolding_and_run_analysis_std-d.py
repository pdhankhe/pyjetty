...
class RunAnalysisAng(run_analysis.RunAnalysis):
  ...
  def initialize_user_config(self):
    ...
    self.use_MC_gauss_info = config['use_MC_gauss_info']
    self.use_PDG_gauss_info = config['use_PDG_gauss_info']
    ...
    self.obs_axis = config['obs_axis']
    print("sparse = ", self.hsparse_jet, ", obs_axis = ", self.obs_axis)
  ...
  def hf_jet_shape_extraction(self):
    ...
    for i in range(len(self.hsparse_jet)):
        sparse = file_data.Get(self.hsparse_jet[i])
        #obs_axis = self.obs_axis
  ...
  def rawJetSpectra(self, sparse, jetR):
    ...
    #no longer necessary -> obs_axis = obs_axis #self.obs_axis
    print('obs_axis = ', self.obs_axis, ', type is ', type(self.obs_axis))
    obs_axis = 3 # self.obs_axis
    ...
    for beta in self.beta_list:
        ...
        for ibin2 in range(len(self.fptbinsJetFinalA)-1):
            dim = array('i',(2,3,1,0)) #axes details from the root file
            #Clone sparse info
            ...
            for ipt in range(self.fptbinsDN):
              ...
              elif self.use_PDG_gauss_info:
                fitterp.SetFixGaussianMean(self.Dmass)
                print("mean value used from PDG")
              else:
                fitterp.SetInitialGaussianMean(self.Dmass)
        ...
        hangvsjetpt_feeddown = self.feeddown(obs_axis, jetR, beta)
        hangvsjetpt_feedsub = hangvsjetpt_eff_corr.Clone("h_jet_axis_JetPt_R%s_%s" % (jetR, beta))
        hangvsjetpt_feedsub.Add(hangvsjetpt_feeddown,-1)
        
        #these commands are not very smart but for a good reason
        #hangvsjetpt_feeddown = hangvsjetpt_eff_corr.Clone("h_jet_axis_JetPt_R%s_%s" % (jetR, beta))
        #hangvsjetpt_feeddown.Add()
        #hangvsjetpt_feeddown.Add(hfeeddown)
        ...
        #for ibinx in range(hangvsjetpt_feeddown.GetNbinsX()):
        #    for ibiny in range(hangvsjetpt_feeddown.GetNbinsY()):
        #        content = hangvsjetpt_feeddown.GetBinContent(ibinx+1,ibiny+1)
        #        if content < 0:
        #            hangvsjetpt_feeddown.SetBinConent(ibinx+1,ibiny+1,0)
        #            hangvsjetpt_feeddown.SetBinError(ibinx+1,ibiny+1,0)

        #bin_arrayfd = []
        #for ibinx in range(hangvsjetpt_feeddown.GetNbinsX()):
        #    bin_arrayfd.append(hangvsjetpt_feeddown.GetBinLowEdge(ibinx+1))
        #print("Feeddown bin edges = ", bin_arrayfd )
        
        #bin_arrayeff = []
        #for ibinx in range(hangvsjetpt_eff_corr.GetNbinsX()):
        #    bin_arrayeff.append(hangvsjetpt_eff_corr.GetBinLowEdge(ibinx+1))
        #print("EffCorr bin edges = ", bin_arrayeff)
        
        name1 = "h_effaxesdiff_JetPt_R%s_%s" % (jetR, beta)
        hangvsjetpt_eff_corr.SetName("h_effaxesdiff")
        name2 = "h_rawaxesdiff_JetPt_R%s_%s" % (jetR, beta)
        hangvsjetpt_sb_sub.SetName("h_rawaxesdiff")
        name3 = "h_feeddown_JetPt_R%s_%s" % (jetR, beta)
        hangvsjetpt_feeddown.SetName("h_feeddown")
        name = "h_jet_axis_JetPt_R%s_%s" % (jetR, beta)
        hangvsjetpt_feedsub.SetName(name)
        
        #print observable bins for reference, from data code
        delta_R = np.linspace(-0.05, 0.8, 172) #need to change bins size
        print(delta_R)

        setattr(self, name1, hangvsjetpt_eff_corr)
        setattr(self, name2, hangvsjetpt_sb_sub)
        setattr(self, name3, hangvsjetpt_feeddown)
        setattr(self, name, hangvsjetpt_feedsub)

        fout = ROOT.TFile("AnalysisResults_%s.root" % (sparse), 'recreate')
        hangvsjetpt_eff_corr.Write()
        hangvsjetpt_sb_sub.Write()
        hangvsjetpt_feeddown.Write()
  ...
  def get_simulated_yields(self, file_path, dim, prompt, xsec, obs_axis):
    ...
    # cut on shape -- looks like it should be lambda_limits
...
###########################################################################
    ...
    name_ch = "hResponse_JetPt_jet_axis_R%s_%sScaledfd" % (jetR, beta)
    print("name_ch = ",name_ch)
    thn_ch = response.Get(name_ch)
    print("thn_ch = ", thn_ch)
    
    name_prompt = "hResponse_JetPt_jet_axis_R%s_%sScaled" % (jetR, beta)
    thn_prompt = response.Get(name_prompt)
    print("thn_prompt = ", thn_prompt)
...
###########################################################################
    ...
    hangvsjetpt_fd_projy = hangvsjetpt_fd.ProjectionY()
    hangvsjetpt_fd_projy.Scale(1/hangvsjetpt_fd_projy.Integral(),"width")
    fout = ROOT.TFile("AnalysisResults_outputfd_{}.root".format(beta), 'recreate')
    hangvsjetpt_fd_projy.Write()
    folded.Write()
