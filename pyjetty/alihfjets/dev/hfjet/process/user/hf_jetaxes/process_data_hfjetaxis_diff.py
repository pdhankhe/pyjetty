...
class HFAnalysisInvMass(hfdio.HFAnalysis):
	def __init__(self, **kwargs):
		...
		self.nbinsWTA=array('i',(60,60,60,350,160))
		self.binlowWTA=array('d',(0,0,0,1.72,0))
		self.binhighWTA=array('d',(60,60,60,2.07,0.8))

		self.fsparseJetWTA=ROOT.THnSparseD("hsparsejetWTA","hsparsejetWTA;Jet_pt;JetWTA_pt;D_pt;D_m;dR_STD_WTA",6,self.nbinsWTA,self.binlowWTA,self.binhighWTA)
		self.fsparseJetWTAvalue=array('d',(0,0,0,0,0))
		self.fsparseJetWTA.Sumw2()

		self.nbinsSD=array('i',(60,60,60,350,160,160))
		self.binlowSD=array('d',(0,0,0,1.72,-0.1,-0.1))
		self.binhighSD=array('d',(60,60,60,2.07,0.7,0.7))

		self.fsparseJetSD=ROOT.THnSparseD("hsparsejetSD","hsparsejetSD;Jet_pt;JetSD_pt;D_pt;D_m;dR_STD_SD;dR_WTA_SD",7,self.nbinsSD,self.binlowSD,self.binhighSD)
		self.fsparseJetSDvalue=array('d',(0,0,0,0,0,0))
	...
	def analysis(self, df):
		...
		for id0, d0 in enumerate(djmm.Ds):
			...
			jetR=0.4
			ja = jet_analysis.JetAnalysis(jet_R = jetR,jet_RecombinationScheme=fj.E_scheme, particle_eta_max=0.9,jet_pt_min=1.0)
			...
			if len(djets) > 0:
				...
				self.fsparseJetWTAvalue[0]=j.perp()
				self.fsparseJetWTAvalue[1]=jet_wta.perp()
				self.fsparseJetWTAvalue[2]=dcand[0].perp()
				self.fsparseJetWTAvalue[3]=dcand[0].m()
				self.fsparseJetWTAvalue[4]=j.delta_R(jet_wta)
				...
				self.fsparseJetSDvalue[2]=dcand[0].perp()
				self.fsparseJetSDvalue[3]=dcand[0].m()
				self.fsparseJetSDvalue[4]=deltaR_STD_SD
				self.fsparseJetSDvalue[4]=deltaR_WTA_SD
