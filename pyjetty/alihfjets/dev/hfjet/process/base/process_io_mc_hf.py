...
import math as ma
import numpy as np
from pyjetty.alihfjets.dev.hfjet.bitwise  import filter_bit_df, tag_bit_df
...
class HFAnalysis(MPBase):
    ...
    def exec_analysis_d0_df(self, df, IsGen):
        #print(df)
        self.pbar.update(1)
        _n_d0s = len(df)
        if _n_d0s < 1:
        ...
        self.D_jet_reconstruction_event_by_event(df, IsGen)
        self.df_tracks = None
    ...
    def apply_cuts_ptbin(self, df_):
        ...
        config_cuts_type = self.config_type
        pt_cand_cuts = self.config["analysisD0"]["cuts"].get(config_cuts_type)
    ...
    def analyze_slower(self):
        ...
        self.d0_event_merged_df = d0ev_df
        ...
        self.d0_ev_tagged_df = self.tagging_prompt_fd_reflection(self.d0_event_merged_df, False)
        self.d0_ev_gen_tagged_df = self.tagging_prompt_fd_reflection(self.d0ev_gen_df, True)
        
        self.group_fjparticles_jet_finding(True, False, 0)

        
    def group_fjparticles_jet_finding(self, group_by_evid=True, random_mass=False, min_pt=0.):
        d0ev_df_grouped = None
        d0ev_gen_df_grouped = None
        d0ev_df_grouped = self.d0_ev_tagged_df.groupby(self.unique_identifier)
        d0ev_gen_df_grouped = self.d0_ev_gen_tagged_df.groupby(self.unique_identifier)
       
  
        # generated level jet
        with tqdm.tqdm(total=len(d0ev_gen_df_grouped)) as self.pbar:
            d0ev_gen_df_grouped.apply(self.exec_analysis_d0_df, True)
        self.pbar.close()
        # reconstructed level level jet
        with tqdm.tqdm(total=len(d0ev_df_grouped)) as self.pbar:
            d0ev_df_grouped.apply(self.exec_analysis_d0_df, False)
        self.pbar.close()
...
class HFAnalysisIO(MPBase):
    ...
    def load_file(self, path):
        ...
        self.Is_treff_sys = False
        #apply treff
        if self.Is_treff_sys:
