...
class HFAnalysis(MPBase):
	...
	def analyze_slower(self):
		...
		if len(d0ev_gen_df_grouped)>0:
			...
			self.fill_generated_info(_pd_gen)
	
		if len(d0ev_df_grouped)>0 and len(d0ev_gen_df_grouped)>0:
			self.fill_reco_info(_pd_reco,_pd_gen)	
